...
- The row is a read-only projection; no new persisted row entity or identity registry.
- Reliable existing book Progress enables Resume.
- Unmatched or uncertain Progress falls back to Details.
- Downloads and audiobooks are optional card decorations only.
- No unified download registry, BookTorrents inventory project, standalone audiobook route, or bulk Progress migration.
...
- Theatre is the structural reference; its video-specific concepts must not leak into Biblio.
...
- Do not invent repository paths, commands, or Lanista capabilities.
