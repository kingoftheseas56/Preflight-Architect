# Local Media Launch Slice 3C — Player 1/libmpv Adapter Reference Implementation Bundle r1

## Status

**Design approved; reference implementation candidate only. Uncompiled, untested, unexecuted, unadopted, and runtime-unverified.**

This immutable bundle supersedes the earlier Slice 3C conflict handoff as the active adoption input. The conflict handoff remains historical evidence. Brotherhood review approved decisions **A1 + B1**, corrected the RHI framing, and supplied the bare-libmpv admission direction.
...
- **Confirmed from pinned source:** default boot selects Player 1/OpenGL; Player 2 is opt-in, SessionStore accepts `target.id` before `target.path`, and existing `playLocalFile()` invokes subtitle discovery and global progress.
