...
This immutable revision supersedes `2026-08-06-colosseum-local-media-launch-slice-1-code-03-router.md` by adding the router's direct `<utility>` dependency. It is the canonical router segment of the interim Slice 1 bundle. The execution agent must inspect, adapt, compile, test, and runtime-validate before adoption.
...
LocalMediaRouteResult LocalMediaRouter::route(
    ...
    const QList<const LocalMediaHandler *> &handlers) const
{
    const LocalMediaInspectionResult inspection = LocalMediaInspector().inspect(source);
    if (!inspection.ok)
    ...
    if (matches.size() > 1) {
        ...
        for (const LocalMediaHandler *handler : matches)
            ids.append(handler->id());

        return LocalMediaRouteResult::failed(LocalMediaError::make(
            LocalMediaErrorCode::AmbiguousHandler,
            QStringLiteral("Colosseum found more than one way to open this file."),
            QStringLiteral("Multiple handlers support '%1': %2")
                .arg(localMediaKindName(classification.kind), ids.join(QStringLiteral(", ")))));
    }
    ...
    if (!prepared.ok) {
        ...
        return LocalMediaRouteResult::failed(LocalMediaError::make(
            ...
            QStringLiteral("Handler '%1' failed without a typed error.")
                .arg(handlerId)));
    }
    ...
    const QVariantMap &descriptor)
{
    const QString appType =
        descriptor.value(QStringLiteral("appType")).toString().trimmed();
    const QString contentKind =
        descriptor.value(QStringLiteral("contentKind")).toString().trimmed();
    const QString title =
        descriptor.value(QStringLiteral("title")).toString().trimmed();
    const QVariantMap target =
        descriptor.value(QStringLiteral("target")).toMap();

    if (appType.isEmpty() || contentKind.isEmpty() || title.isEmpty()
        || target.isEmpty()) {
        return LocalMediaError::make(
            LocalMediaErrorCode::InvalidSessionDescriptor,
            QStringLiteral("Colosseum could not prepare that file."),
            QStringLiteral(
                "A SessionStore descriptor requires non-empty appType, "
                "contentKind, title, and target fields."));
    }
    ...
    return LocalMediaError::make(
        ...
        QStringLiteral("Colosseum could not prepare that file."),
        QStringLiteral(
            "The SessionStore target requires showKey, id, path, or infoHash "
            "as a provisional identity."));
}
